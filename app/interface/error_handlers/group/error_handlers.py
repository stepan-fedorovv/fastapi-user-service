...
from app.domain.errors.permission_error_classes import (
    PermissionNotFound,
    PermissionAlreadySetInGroup,
)
...
def group_install_exception_handlers(app: FastAPI) -> None:
    ...
    async def _permission_already_set(
        request: Request, exc: PermissionAlreadySetInGroup
    ):
        return JSONResponse(
            status_code=status.HTTP_409_CONFLICT,
            content=exc.errors_dto.model_dump(),
